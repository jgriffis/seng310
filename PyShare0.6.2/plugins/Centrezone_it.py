...
import gettext
gettext.bindtextdomain('sendToCenterzone', '.')
gettext.textdomain('sendToCenterzone')
_ = gettext.gettext
...
acceptImageExtensions = ["jpg", "jpeg", "gif", "png"]
...
class __Centerzone():
    ...
    def upload(self, file, semaphore, callbackFunction):
        ...
        curl.setopt(pycurl.URL, "http://upload.centerzone.it/upload.php")
        curl.setopt(pycurl.HTTPHEADER, ["Except:"])
        curl.setopt(pycurl.POST, 1)
        curl.setopt(pycurl.HTTPPOST, [('userfile[]', (pycurl.FORM_FILE, file)), ('private_upload', '0'), ('upload_type','standard')])
...
Please try again later. If the problem still occur try sending file manually by website."""), _("Upload unsuccessful"))
        ...
        #db.addFile("Centerzone",basicLinks[0], basicLinks[1], basicLinks[2], os.path.basename(file), os.path.getsize(file),file.split('.')[-1].lower(), getMimeTypes(file))
...
def uploadFile(file, resultCallbackFunction=printLink, progressCallbackFunction=printProgressToCLI, fileNumber = 0, semaphore = BoundedSemaphore(2), username="", password=""):
    sti = __Centerzone(fileNumber, progressCallbackFunction)
